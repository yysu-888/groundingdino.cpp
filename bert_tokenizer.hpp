...
namespace bert_tokenizer {
    ...
    public:
        ...
        std::vector<std::wstring> tokenize(const std::string& text) const;
        std::vector<int64_t> convertTokensToIds(const std::vector<std::wstring>& text) const;

        std::vector<int64_t> encode(const std::string& text, int max_length, bool add_special_token = true, bool padding = true) const;
    ...
    bool BasicTokenizer::isPunctuation(const wchar_t& ch) const {
        if ((ch >= 33 && ch <= 47) || (ch >= 58 && ch <= 64) ||
            ...
            return true;
        auto cat = utf8proc_category(ch);
        if (cat == UTF8PROC_CATEGORY_PD || cat == UTF8PROC_CATEGORY_PS || cat == UTF8PROC_CATEGORY_PE || cat == UTF8PROC_CATEGORY_PC || cat == UTF8PROC_CATEGORY_PO  // sometimes ¶ belong SO
    ...
    std::vector<int64_t> FullTokenizer::encode(const std::string& text, int max_length, bool add_special_token, bool padding) const {
        auto tokens              = tokenize(text);
        std::vector<int64_t> ids = convertTokensToIds(tokens);
    ...
    std::vector<int64_t> FullTokenizer::convertTokensToIds(const std::vector<std::wstring>& text) const {
